      ...
      <main className={styles.main}>
        ...
        <div className={styles.grid}>
          ...
          {aboutEmployees && aboutEmployees.map((aboutdata, index) => {
            ...
            return (
              <employeeCard key={about} firstname={aboutdata.firstName} jobtitle={aboutdata.jobTitle} color="red" font="25px"/>
            )
          }
          })}
           <div>Computing Departments:</div>
          {aboutEmployees && aboutEmployees.map((aboutdata, index) => {
            if(aboutdata.department.toLowerCase() === "human resources") {
            return (
              <employeeCard key={about} department={aboutdata.department} color="blue" font="25px"/>
            )
          }
          })}
        </div>
